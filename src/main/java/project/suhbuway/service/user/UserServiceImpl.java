...
public class UserServiceImpl implements UserService {
	...
	public int joinUser(User user) {
		...
		if (result == 0)
			throw new RuntimeException("가입되지 않았습니다.");

		// 가입 성공하면 권한 부여(추가)
		// result = authoritiesDAO.insertAuthority( new Authority( member.getId(),
		// RoleConstant.ROLE_MEMBER ) );
		// if( result ==0 ) throw new RuntimeException("권한 부여에 오류가 발생하여 Rollback
		// 되었습니다.--1");
